...
% University of São Paulo - Instituto de Matemática e Estatística
% (romenig@gmail.com)
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\author[Romenig, Tulio, Leônidas and Anarosa]{Romenig Ribeiro\inst{1},  Tulio Faria\inst{2}, Leônidas Brandão\inst{1} and Anarosa~Brandão\inst{3}} 
\institute[] 
{
...
\begin{frame}
\frametitle{University of São Paulo}
\begin{figure} %IME%
...
\item Challenges that are currently faced by teacher and students of STEM courses:
	...
	\item{new way of thinking (to solve problems computationally)}
	\end{itemize}
\end{itemize}
\end{block}
\end{frame}
...
\item Most of Visual Programming (\textbf{VP}) systems allows students to:
	\begin{itemize}
	\item{compile and execute their algorithm with a single click}
	\item{build programs without any concern related to the textual  programming language syntax}
...
\item Workload: defined in \cite{p5} as a hypothetical construct that represents the cost of someone finishing a task and reaching a certain level of performance.
...
Volunteers registered through web. The propaganda was performed for a short period of time (4 weeks), and mainly restricted to the University of São Paulo (USP).
\begin{table}[h]
...
\begin{itemize}
\item G2 group: (C+VPL, textual programming), were up to 15 times and it was not uncommon to find a number greater than five attempts
...
\begin{itemize}
\item G2 group: the number of submissions was slightly higher, however, the number of attempts reached the maximum of 17
\item G1 group: regarding the number of submissions for G1 few students did the activities, however, their submission was correct on only 1 attempt
\end{itemize}
\end{block}
\end{frame}
%------------------------------------------------
...
\begin{frame}
\frametitle{Discussion, conslusions and future work}
...
The survey questionnaire was answered by 26 students (23 accessed the system and 3 did not). The main reason to frustrations were problems with the Java applet technology and the difficulties with its configuration.
\end{block}
\end{frame}
%------------------------------------------------
\begin{frame}
...
\bibitem[1]{p1} [1] Carlisle, M. C. (2009)
...
\bibitem[3]{p3}[3] Maloney, J., Resnick, M., Rusk, N., Silverman, B., Eastmond, E. (2010)
\newblock The Scratch programming language and environment
\newblock \emph{Transactions on Computing Education (TOCE)}, Vol. 10, issue 4, November 2010.

\bibitem[4]{p4}[4] Cooper, S., Dann, W., Pausch, R. (2000)
...
\footnotesize{
\begin{thebibliography}{99} % Beamer does not support BibTeX so references must be inserted manually as below
\bibitem[5]{p5}[5] Hart, S. G., Staveland, L. E.  (1988)
